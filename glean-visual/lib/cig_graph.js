...
VisualCIG.prototype._filteredHierLinks = null;
...
VisualCIG.prototype.findNode = function(name) {
	return this._data.nodes.find((d) => d.name == name);
}
...
VisualCIG.prototype.showFromData = function(data, config, callback) {
	...
	// this._propagateToDescendants(this._data);
	...
	this._simul = d3.forceSimulation()
		.force("link", d3.forceLink().id((d) => d.id))//.distance(50))
		.force("charge", d3.forceManyBody().strength(-2000))
		.force("center", d3.forceCenter(width / 2, height / 2))
		// seems to do more harm than good in our case
...
VisualCIG.prototype._transitTo = function(node, workflowState, decisionState, propagateDown, propagateUp) {
	if (workflowState)
		node.workflow_state = workflowState
	if (decisionState)
		node.decisional_state = decisionState
...
VisualCIG.prototype._addNodes = function(adds) {
	adds.forEach((add) => {
		var children = add.parent.children; 
		if (add.before) {
...
VisualCIG.prototype._showTree = function() {
	...
	this._data.nodes.forEach((d) => d.cig = this);
	
	this._resetTreeLinks();
	this._showTreeLinks(this._data.links);
	this._showTreeNodes(this._data.nodes);
...
VisualCIG.prototype._showTreeNodes = function(nodesData, update) {
	...
	nodes.filter((d) => d.node_type == 'task_node' && d.children !== undefined)
		  .append('circle')
		  .classed('node', true)
	...
	nodes.filter((d) => d.node_type == 'decision_node')
		  .append('path')
		  .classed('node', true);
	...
	nodes.insert("rect", "text")
		.classed('captionBox', true)
		.attr("width", (d) => d.bbox.width)
		.attr("height", (d) => d.bbox.height - 1)
		.style("fill", "white");
	
	const allNodes = nodes.selectAll("circle.node,rect.node,path.node");
	
	const config = this._config;
	allNodes.filter((d) => (d && d.node_type))
		.on("mouseover", (e, i) => this._tooltip_onMouseOver(e, i, config))
		.on("mouseout", this._tooltip_onMouseOut);
		
	allNodes.filter((d) => (d && d.node_type && (d.description || d.source)))
...
VisualCIG.prototype._showTreeLinks = function(hierLinksData) {
	...
	const filteredLinks = hierLinks.filter((d) => {
			return this._data.nodes[d.target].after === undefined && 
				this._data.nodes[d.target].hidden === undefined // don't show hidden elements 
		})
		.append('line')
		.classed('link', true)
	...
	showLineCaption(
		hierLinks.filter((d) => d.condition !== undefined),
		(d) => d.condition
	);

	showLineCaption(
		hierLinks.filter((d) => d.condition === undefined 
			&& this._data.nodes[d.target].condition !== undefined
		),
		(d) => this._data.nodes[d.target].condition
	);
	
	showLineCaption(
		hierLinks.filter((d) => this._data.nodes[d.target].after !== undefined),
		(d) => "after"
	);
	
	setupLineArrow(this._svg, "tree_arrow", [ 8, 8 ], "black");

	this._filteredHierLinks = filteredLinks;
	this._hierLinks = hierLinks;
}
...
VisualCIG.prototype._tick = function(e) {
	// push further apart at start of layout'ing
	const k = 10 * this._simul.alpha();

	// push sources up and targets down to form a weak tree.
	this._filteredHierLinks
		.each(function(d) {
	...
	this._hierLinks.selectAll("text.hierCaption")
		.attr('x', (d, i) => linkCentroidX(d))
		.attr('y', (d, i) => linkCentroidY(d) + 10)
		.call(getBB);
  	this._hierLinks.selectAll("rect.captionBox")
		.attr("x", (d) => d.bbox.x)
	  	.attr("y", (d) => d.bbox.y + 1);
	...
	allNodes.filter((d) => d.node_type == 'task_node')
		  .attr('cx', (d) => d.x)
		  .attr('cy', (d) => d.y);
	...
	allNodes.filter((d) => d.node_type == 'decision_node')
		  .attr('d', (d) => downTriangle(this._settings.nodeStyle.triangle.width, 
				this._settings.nodeStyle.triangle.height, d.x, d.y));
		  
	// -- captions
	
	this._nodes.selectAll("text.nodeCaption")
		  .attr('x', (d) => d.x)
		  .attr('y', (d) => d.y + 24)
		  .call(getBB);
	this._nodes.selectAll("rect.captionBox")
		.attr("x", (d) => d.bbox.x)
		.attr("y", (d) => d.bbox.y + 1);

	// simple positioning (regardless of node type)
	/*this._nodes
		.attr("cx", function(d) {
			return d.x;
		})
...
VisualCIG.prototype._formatNodesOfType = function(nodes, type) {
	nodes.selectAll("circle.node,rect.node,path.node")
		.filter((d) => (d && d.node_type))
		.style('stroke', 'black')
		.style('fill', (d) => { 
		  switch (d.decisional_state) {
			case 'notChosenState':
				return this._settings.decisionalState.node.fill[d.decisional_state];
			default:
		...
		.style("stroke-dasharray", (d) => {
		   switch (d.decisional_state) {
				case 'notChosenState':
					return this._settings.decisionalState.node.stroke_dasharray[d.decisional_state];
				default:
					const state = d.workflow_state;
...
VisualCIG.prototype._formatHierLinks = function(hierLinks) {
	hierLinks.selectAll('line')
		.style('stroke', (d) => "black")
		.style("stroke-dasharray", (d) => {
			const state = this._data.nodes[d.target].decisional_state;
...
VisualCIG.prototype._tooltip_onMouseOver = function(e, i, config) {
	...
	var wfState = cig._stateLabel(i.workflow_state);
	var dcState = cig._stateLabel(i.decisional_state);
...
VisualCIG.prototype._infoBox_onClick = function(d, i) {	
	...
	if (i.source !== undefined)
		descr = `<div><b>Source</b>: ${i.source}</div>`;
	
	else { 
		if (i.taskSource !== undefined)
			descr = `<div class="source"><b>Source</b>: ${i.taskSource}</div>`;
		else if (i.workflowSource !== undefined)
			descr = `<div class="source"><b>Source</b>: ${i.workflowSource}</div>`;
	}
